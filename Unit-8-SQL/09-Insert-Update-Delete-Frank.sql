...
INSERT into pet_types 
       (name , species) 
values('Siamese', 'Cat'); -- pet_type-id not specified as it is auto generated
;
...
-- name  	pet_type   			owner
-- Jena  	German Shepard Dog 	Frank
-- Princess Tea Cup Dog   		Jay
-- Chloe 	Siamese				Cameron


-- pet_type_id was automatically generated by DBMS
-- We dont really know what it is 
-- But... data base manager knows
...
-- (a syb-query is just a select inside another SELECT,INSERT,UPDATE,DELETE)
--
--Ask the data base manager to get the primary key value for the data that we are inserting 
-- To get the pet_type_id for a german shepeard dog the select would be:
-- select pet_type_id from pet_types where name  'German shepard' and species = 'Dog';
-- select owner_id from owner whwere first_name = 'Frank' and last_name = 'Fella;'
...
Values('Chloe', 
		...
		(select owner_id from owner where first_name = 'Cam' )
...
--(name, pet_type_id, owner_id)
--Values('Mel', 
	--	(select pet_type_id from pet_types where name = 'Great Dane' and species = 'Dog' ),
	--	(select owner_id from owner where first_name = 'Kevin' )
